...
bool pirActive(PIR* pir) {
    ...
    return (pir->activatedOn > 0) &&
        ((unsigned long)(millis() - pir->activatedOn) < ((unsigned long)(pir->timeout - PIRSensorTimeout)) * (unsigned long)1000);
}
