 ...
 * Контроль активности сервера Home Assistant с поддержка режима "управляемого" устройства позволяет изменять логику работы IoT устройства
 при недоступности сервера УД.
 * Поддержка OTA (On The Air update, прошивка устройства по WiFi)
 * Модуль обработки кнопок: защита от дребезга, 
 различные типы событий (нажатие/отпускание, короткие/длинные нажатия), обработка комбинаций кнопок, публикация событий кнопок в MQTT
 * Модуль управления реле: ограничение частоты переключений, взаимодействие через MQTT
 * Диммер: одноканальный, двухканальный и WC/WW режим работы, взаимодействие через MQTT
