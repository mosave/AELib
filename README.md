 ...
 * **\<Relay\>/State**: "0" или "1", Текущее состояние реле (retained)
 
Топики, обрабатываемые модулем: 
 * **\<Relay\>/State**, payload "0" или "1": задать текущее состояние реле
 * **\<Relay\>/Switch**: переключить реле в другое состояние
 * **\<Relay\>/SetName**, payload = "New Relay Name": переименовать реле, если оно не было жестко определено при регистрации
