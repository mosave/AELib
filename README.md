 ...
 * **<<Button1>>/State**: "0" или "1", текущее состояние кнопки (retained)
 * **Button1/ShortPressed**: отправляется однократно при обнаружении короткого нажатия
 * **Button1/LongPressed**: отправляется однократно при обнаружении длинного нажатия
 * **Button1/VeryLongPressed**: отправляется однократно при обнаружении очень длинного (более 10с) нажатия
...
будет иметь имя в формате "RelayX" и в дальнейшем может быть переименовано по MQTT.

Публикуемые модулем топики:
 * **Relay1/State**: "0" или "1", Текущее состояние реле (retained)
 
Топики, обрабатываемые модулем: 
 * **Relay1/State**, payload "0" или "1": задать текущее состояние реле
 * **Relay1/Switch**: переключить реле в другое состояние
 * **Relay1/SetName**, payload = "New Relay Name": переименовать реле, если оно не было жестко определено при регистрации
